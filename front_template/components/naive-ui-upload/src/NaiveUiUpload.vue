    ...
    <div>
        ...
        >
            <slot>
                <NButton
                    v-if="
                        getProps()['list-type'] === 'text' ||
                        getProps()['listType'] === 'text'
                    "
                    >{{ uploadText }}</NButton
                >
            </slot>
...
const accept = attrs.accept
    ? (attrs.accept as string).replace(/\s/g, "").toLowerCase()
    : undefined
...
async function handleConfirmCropper() {
    ...
    const [err, { url }] = await to<string>(uploadApi(file))
    cropperLoading.value = false
    if (err) {
        return false
    }
    currentCropperFile.value.file = file
    currentCropperFile.value.url = url
...
async function handleCustomRequest({
    ...
    const [err, { url }] = await to(uploadApi(file.file as File, onProgress))
    if (err) {
        onError()
        return
    }
