...
//框架页面组件
const packagesViews: Record<string, RouterComponent> = import.meta.glob(
    "@/packages/view/**/*.vue",
    { eager: true }
)
...
function hasWhiteRouter(path: string) {
    const appStore = appPinia()
    return appStore.configOptions.whiteList.some(
        (e: string) => path.indexOf(e) === 0
    )
}
...
function hasUserInfo() {
    const appStore = appPinia()
    return appStore.userInfo?.userId
}
